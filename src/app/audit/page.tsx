...
export default function AuditLogPage() {
  ...
  const launchNames = useMemo(() => {
    ...
    for (const l of launches) map[l.id] = l.name;
    return map;
  }, [launches]);
  ...
  const filteredEvents = useMemo(() => {
    ...
    return allEvents.filter(e => e.event_type === filterType);
          ...
          >
            ...
            {eventTypes.map(t => (
              <option key={t} value={t}>
                {getEventConfig(t).label} ({allEvents.filter(e => e.event_type === t).length})
              </option>
            ))}
          </select>
        </div>
