...
export default function LaunchDetailClient({ launch, reviews, events, user }: LaunchDetailClientProps) {
  ...
  // Owners — use launch creator name from events or fallback
  const creatorEvent = events.find(e => e.event_type === 'LAUNCH_CREATED');
  const owners = [{
    id: launch.created_by,
    display_name: creatorEvent?.performed_by_name || 'Launch Owner',
    email: '',
    role: 'admin' as const,
