...
export default function SettingsPage() {
  ...
  const users = [store.getCurrentUser()]; // MVP: just current user
  const reviewDefs = store.getReviewDefinitions();
    ...
    <div>
      ...
      <div className="settings-card">
        ...
        <p className="text-sm" style={{ color: 'var(--text-tertiary)', marginTop: 8 }}>
          Organization settings will be editable once backend is connected.
        </p>
      </div>
    </div>
  );
...
function ReviewSettings({ reviewDefs }: { reviewDefs: ReturnType<typeof store.getReviewDefinitions> }) {
  return (
    <div>
...
function TeamSettings({ users }: { users: ReturnType<typeof store.getCurrentUser>[] }) {
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
